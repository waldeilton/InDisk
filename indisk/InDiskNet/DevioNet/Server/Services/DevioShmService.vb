Imports LTR.IO.InDisk.Devio.INDPROXY_CONSTANTS
Imports LTR.IO.InDisk.Devio.Server.Providers

Namespace Server.Services

  ''' <summary>
  ''' Class that implements server end of InDisk/Devio shared memory based communication
  ...
  Public Class DevioShmService
    ...
    Public Shared ReadOnly Property DefaultBufferSize As Long
      Get
        If Environment.OSVersion.Version.Major > 5 Then
          Return 2097152 + INDPROXY_HEADER_SIZE
        Else
          Return 1048576 + INDPROXY_HEADER_SIZE
        End If
      End Get
    End Property
    ...
    ''' <summary>
    ''' Runs service that acts as server end in InDisk/Devio shared memory based communication. It will first wait for
    ...
    Public Overrides Sub RunService()
      ...
      Try
        ...
        Using MapView
          ...
          Do
            Dim RequestCode = MapView.Read(Of INDPROXY_REQ)(&H0)

            'Trace.WriteLine("Got client request: " & RequestCode.ToString())

            Select Case RequestCode

              Case INDPROXY_REQ.INDPROXY_REQ_INFO
                SendInfo(MapView)

              Case INDPROXY_REQ.INDPROXY_REQ_READ
                ReadData(MapView)

              Case INDPROXY_REQ.INDPROXY_REQ_WRITE
                WriteData(MapView)

              Case INDPROXY_REQ.INDPROXY_REQ_CLOSE
    ...
    Private Sub SendInfo(MapView As SafeBuffer)
      ...
      Dim Info As INDPROXY_INFO_RESP
      Info.file_size = CULng(DevioProvider.Length)
      Info.req_alignment = CULng(REQUIRED_ALIGNMENT)
      Info.flags = If(DevioProvider.CanWrite, INDPROXY_FLAGS.INDPROXY_FLAG_NONE, INDPROXY_FLAGS.INDPROXY_FLAG_RO)
    ...
    Private Sub ReadData(MapView As SafeBuffer)
      ...
      Dim Request = MapView.Read(Of INDPROXY_READ_REQ)(&H0)
      ...
      Dim Response As INDPROXY_READ_RESP

      Try
        If ReadLength > MapView.ByteLength - INDPROXY_HEADER_SIZE Then
          Trace.WriteLine("Requested read length " & ReadLength & ", lowered to " & CInt(MapView.ByteLength - CInt(INDPROXY_HEADER_SIZE)) & " bytes.")
          ReadLength = CInt(MapView.ByteLength - CInt(INDPROXY_HEADER_SIZE))
        End If
        Response.length = CULng(DevioProvider.Read(MapView.DangerousGetHandle(), INDPROXY_HEADER_SIZE, ReadLength, Offset))
        Response.errorno = 0
    ...
    Private Sub WriteData(MapView As SafeBuffer)
      ...
      Dim Request = MapView.Read(Of INDPROXY_WRITE_REQ)(&H0)
      ...
      Dim Response As INDPROXY_WRITE_RESP

      Try
        If Length > MapView.ByteLength - INDPROXY_HEADER_SIZE Then
          Throw New Exception("Requested write length " & Length & ". Buffer size is " & CInt(MapView.ByteLength - CInt(INDPROXY_HEADER_SIZE)) & " bytes.")
        End If
        Length = DevioProvider.Write(MapView.DangerousGetHandle(), INDPROXY_HEADER_SIZE, Length, Offset)
        Response.errorno = 0
    ...
    Protected Overrides ReadOnly Property InDiskProxyModeFlags As InDiskFlags
      Get
        Return InDiskFlags.TypeProxy Or InDiskFlags.ProxyTypeSharedMemory
      End Get
    End Property
