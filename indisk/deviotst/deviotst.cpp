...
wmain(int argc, LPWSTR *argv)
{
  ...
  if (argc < 4)
    {
      ULONGLONG req = INDPROXY_REQ_INFO;
      if (!Overlapped.BufSend(hIO, &req, sizeof(req)))
	{
      ...
      INDPROXY_INFO_RESP info_resp = { 0 };
      ...
      if (!Overlapped.BufSend(hIO, &read_req, sizeof read_req))
	{
      ...
      INDPROXY_READ_RESP read_resp = { 0 };
      ...
      if (!Overlapped.BufSend(hIO, buf, (DWORD) write_req.length))
	{
      ...
      INDPROXY_WRITE_RESP write_resp = { 0 };
...
wmainCRTStartup()
{
  ...
  if (argv == NULL)
    {
      MessageBoxA(NULL, "This program requires Windows NT/2000/XP.", "InDisk",
		  MB_ICONSTOP);
      ExitProcess((UINT)-1);
    }
